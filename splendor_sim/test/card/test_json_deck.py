...
class TestJsonDeck(unittest.TestCase):
    def setUp(self):
        self._validator_patcher = mock.patch(
            "splendor_sim.src.card.json_deck.JsonDeck._JSON_VALIDATOR", autospec=True
        )
        ...
        self._deck_patcher = mock.patch(
            "splendor_sim.src.card.deck.Deck.__init__", autospec=True
        )
        ...
        self._name_to_card_dict = {
            "card %d" % i: card for i, card in enumerate(self._mock_cards)
        }
        ...
        self._mock_json = {
            ...
            "cards": [card.get_name() for card in self._mock_cards],
        }
        ...
        self._mock_card_manager.get_card_by_name.side_effect = lambda name: self._name_to_card_dict[
            name
        ]
    ...
    def test_json_deck_init(self):
        # Arrange
        # Act
        object_pointer = json_deck.JsonDeck(self._mock_tier, self._mock_cards)
        # Assert
        self._mock_deck.assert_called_once_with(
            object_pointer, self._mock_tier, self._mock_cards
        )
    ...
    def test_json_deck_type_get_json_schema(self):
        ...
        # Assert
        self.assertEqual(
            json_schemas.JSON_DECK_SCHEMA, json_deck.JsonDeck.get_json_schema()
        )
