...
class TestJsonCoinReserve(unittest.TestCase):
    ...
    def set_up_json_coin_type_manager(self):
        self._mock_json_coin_type_manager = mock.create_autospec(
            spec=json_coin_type_manager.JsonCoinTypeManager, spec_set=True
        )
        ...
        self._mock_json_coin_type_manager_build_from_json = (
            self._json_coin_type_manager_build_from_json_patcher.start()
        )
        ...
        self._mock_json_coin_type_manager.get_coin_by_name.side_effect = lambda x: self._coin_name_map[
            x
        ]
    ...
    def setUp(self):
        ...
        self._mock_coin_type_manager_json = {"coin_type_manager_json": "here"}
        self._mock_coin_stocks = [
            {"coin_name": self._mock_coin_types[0].get_name(), "count": 0},
            {"coin_name": self._mock_coin_types[1].get_name(), "count": 1},
            {"coin_name": self._mock_coin_types[2].get_name(), "count": 2},
            {"coin_name": self._mock_coin_types[3].get_name(), "count": 3},
            {"coin_name": self._mock_coin_types[4].get_name(), "count": 4},
            {"coin_name": self._mock_coin_types[5].get_name(), "count": 5},
        ]
    ...
    def test_json_coin_reserve_init(self):
        # Arrange
        # Act
        object_pointer = json_coin_reserve.JsonCoinReserve(self._mock_coin_type_manager)
        # Assert
        self._mock_coin_reserve_init.assert_called_once_with(
            object_pointer, self._mock_coin_type_manager, None
        )
    ...
    def test_json_coin_reserve_init_coin_stocks(self):
        ...
        object_pointer = json_coin_reserve.JsonCoinReserve(
            self._mock_coin_type_manager, self._mock_coin_stocks_by_object_pointer
        )
        # Assert
        self._mock_coin_reserve_init.assert_called_once_with(
            object_pointer,
    ...
    def test_json_coin_reserve_build_from_json_invalid_coin_name_not_in_manager(self):
        ...
        self._mock_coin_stocks.append(
            {"coin_name": "not a name in the manager", "count": 0}
        )
        # Act
        # Assert
        with self.assertRaises(ValueError):
    ...
    def test_json_coin_reserve_get_json_schema(self):
        ...
        # Assert
        self.assertEqual(
            object_pointer.get_json_schema(), json_schemas.JSON_COIN_RESERVE_SCHEMA
        )
