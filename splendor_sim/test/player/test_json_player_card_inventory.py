...
class TestJsonPlayerCoinInventory(unittest.TestCase):
    ...
    def set_up_cards(self):
        self._mock_cards = [
            mock.create_autospec(spec=i_card.ICard, specset=True) for _ in range(6)
        ]
    ...
    def set_up_player_card_inventory(self):
        ...
        self._mock_player_card_inventory_init = (
            self._player_card_inventory_patcher.start()
        )
    ...
    def setUp(self):
        ...
        self._mock_card_manager.get_card_by_name.side_effect = lambda name: self._card_name_map[
            name
        ]
    ...
    def test_json_player_card_inventory_init(self):
        ...
        object_pointer = json_player_card_inventory.JsonPlayerCardInventory(
            4, set(self._mock_cards[:2]), set(self._mock_cards[2:])
        )
        # Assert
        self._mock_player_card_inventory_init.assert_called_once_with(
            object_pointer, 4, set(self._mock_cards[:2]), set(self._mock_cards[2:])
        )
    ...
    def test_json_player_card_inventory_build_from_json_invalid_card_name_not_in_manager(
        self
    ):
        # Arrange
    ...
    def test_json_player_card_inventory_build_from_json_invalid_reserved_card_name_not_in_manager(
        self
    ):
        # Arrange
    ...
    def test_json_player_card_inventory_get_json_schema(self):
        ...
        # Assert
        self.assertEqual(
            object_pointer.get_json_schema(), json_schemas.JSON_PLAYER_CARD_INVENTORY
        )
