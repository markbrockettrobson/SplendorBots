...
class TestJsonPlayerCoinInventory(unittest.TestCase):
    ...
    def set_up_sponsors(self):
        self._mock_sponsors = [mock.create_autospec(spec=i_sponsor.ISponsor, specset=True) for _ in range(6)]
        ...
        self._sponsor_name_map = {sponsor.get_name(): sponsor for sponsor in self._mock_sponsors}
    ...
    def set_up_player_sponsor_inventory(self):
        ...
        self._mock_player_sponsor_inventory_init = self._player_sponsor_inventory_patcher.start()
    ...
    def setUp(self):
        ...
        self._mock_sponsor_manager = mock.create_autospec(spec=i_sponsor_manager.ISponsorManager, spec_set=True)

        self._mock_game_state.get_sponsor_reserve.return_value = self._mock_sponsor_reserve
        self._mock_sponsor_reserve.get_sponsor_manager.return_value = self._mock_sponsor_manager

        self._mock_sponsor_manager.get_sponsor_by_name.side_effect = lambda name: self._sponsor_name_map[name]
    ...
    def test_json_player_sponsor_inventory_build_from_json_invalid_card_name_not_in_manager(self):
        # Arrange
    ...
    def test_json_player_sponsor_inventory_get_json_schema(self):
        ...
        # Act
        # Assert
        self.assertEqual(object_pointer.get_json_schema(), json_schemas.JSON_PLAYER_SPONSOR_INVENTORY)
