...
class TestPlayerCardInventory(unittest.TestCase):
    def setUp(self):
        ...
        self._mock_discount = mock.create_autospec(
            spec=i_coin_type.ICoinType, spec_set=True
        )
        self._mock_cost = {
            mock.create_autospec(spec=i_coin_type.ICoinType, spec_set=True): 1
            for _ in range(3)
        }
    ...
    def test_player_card_inventory_init_valid(self):
        ...
        test_player_card_inventory = player_card_inventory.PlayerCardInventory(
            self._max_reserved_cards, set(), set()
        )
        # Assert
        ...
        self.assertEqual(
            test_player_card_inventory.get_max_number_of_reserved_cards(),
    ...
    def test_player_card_inventory_init_valid_reserved_cards(self):
        ...
        # Act
        test_player_card_inventory = player_card_inventory.PlayerCardInventory(
            self._max_reserved_cards, {self._mock_card_list[0]}, set()
        )
        # Assert
        self.assertEqual(test_player_card_inventory.get_total_discount(), {})
        self.assertEqual(test_player_card_inventory.get_victory_points(), 0)
        self.assertEqual(test_player_card_inventory.get_card_set(), set())
        self.assertEqual(
            test_player_card_inventory.get_reserved_card_set(),
    ...
    def test_player_card_inventory_init_valid_cards(self):
        ...
        # Act
        test_player_card_inventory = player_card_inventory.PlayerCardInventory(
            self._max_reserved_cards, set(), {self._mock_card_list[0]}
        )
        # Assert
        self.assertEqual(
            test_player_card_inventory.get_total_discount(), {self._mock_discount: 1}
        )
        self.assertEqual(test_player_card_inventory.get_victory_points(), 1)
        self.assertEqual(
            test_player_card_inventory.get_card_set(), {self._mock_card_list[0]}
        )
        self.assertEqual(test_player_card_inventory.get_reserved_card_set(), set())
        self.assertEqual(test_player_card_inventory.get_number_of_reserved_cards(), 0)
        self.assertEqual(
    ...
    def test_player_card_inventory_init_invalid_to_many_reserved_cards(self):
        ...
        with self.assertRaises(ValueError):
            _ = player_card_inventory.PlayerCardInventory(
                self._max_reserved_cards, set(self._mock_card_list[:4]), set()
            )
    ...
    def test_player_card_inventory_get_reserved_card_list_immutability(self):
        ...
        # Assert
        self.assertEqual(
            test_player_card_inventory.get_reserved_card_set(), pre_mutation
        )
    ...
    def test_player_card_inventory_get_number_of_reserved_cards(self):
        ...
        for i in range(3):
            ...
            # Assert
            self.assertEqual(
                test_player_card_inventory.get_number_of_reserved_cards(), i + 1
            )
    ...
    def test_player_card_inventory_remove_from_reserved_card_list_card_not_in_reserve(
        self
    ):
        # Arrange
        test_player_card_inventory = player_card_inventory.PlayerCardInventory(
        ...
        # Assert
        with self.assertRaises(ValueError):
            test_player_card_inventory.remove_from_reserved_card_set(
                self._mock_card_list[-1]
            )
        ...
        self.assertEqual(
            {
                "max_reserved_cards": self._max_reserved_cards,
                "reserved_cards": [
                    card.get_name() for card in set(self._mock_card_list[:1])
                ],
