...
class TestJsonSponsorReserve(unittest.TestCase):
    def setUp(self):
        ...
        self._mock_sponsor_name_map = {sponsor.get_name(): sponsor for sponsor in self._mock_sponsors}
        self._mock_sponsors_manager = mock.create_autospec(spec=i_sponsor_manager.ISponsorManager, spec_set=True)
        self._mock_sponsors_manager.is_sponsor_in_manager_by_name.side_effect = \
            lambda x: x in self._mock_sponsor_name_map.keys()
        self._mock_sponsors_manager.get_sponsor_by_name.side_effect = \
            lambda x: self._mock_sponsor_name_map[x]

        self._mock_json_sponsor_manager.build_from_json.return_value = self._mock_sponsors_manager
        ...
        self._mock_json = {
            ...
            "sponsors": [
                sponsor.get_name() for sponsor in self._mock_sponsors
            ]
        }
    ...
    def test_json_sponsor_reserve_init(self):
        # Arrange
        # Act
        object_pointer = json_sponsor_reserve.JsonSponsorReserve(self._mock_sponsors_manager, self._mock_sponsors)
        # Assert
